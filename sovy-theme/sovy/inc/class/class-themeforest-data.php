...
class Themeforest_Data {
	...
	public function theme_wizard() {
		if ( get_option( 'sovy_wizard_setup_done' ) !== 'yes' ) {
			add_theme_page(
				'Wizard Setup',
				'Wizard Setup',
				'manage_options',
	...
	private function is_wizard_done() {
		return isset( $_GET['page'] ) && isset( $_GET['wizard_setup_done'] ) && $_GET['page'] === 'sovy-dashboard' && $_GET['wizard_setup_done'] === 'yes';
	}

	public function theme_redirect() {
		if ( get_option( 'sovy_wizard_init_done' ) !== 'yes' ) {
			update_option( 'sovy_wizard_init_done', 'yes' );
			wp_safe_redirect( admin_url( 'admin.php?page=sovy-wizard' ) );
			exit;
		}

		if ( $this->is_wizard_done() ) {
			update_option( 'sovy_wizard_setup_done', 'yes' );
			wp_safe_redirect( admin_url( 'themes.php?page=sovy-dashboard' ) );
		}
	}
				...
				'permission_callback' => function () {
					if ( ! current_user_can( 'manage_options' ) ) {
						return new WP_Error(
							...
							esc_html__( 'Forbidden Access', 'sovy' ),
							array( 'status' => 403 )
						);
					}
	...
	public function enqueue_backend() {
		wp_enqueue_style(
			'sovy-inter-font',
			SOVY_URI . '/assets/dashboard-fonts/inter/inter.css',
			array(),
			SOVY_VERSION
		);

		wp_enqueue_style(
			'sovy-jakarta-sans-font',
			SOVY_URI . '/assets/dashboard-fonts/plus-jakarta-sans/plus-jakarta-sans.css',
			array(),
			SOVY_VERSION
		);
	}
